A model is a function; a system is a model in a loop. This chapter is about the loops: retrieval that feeds the context window with relevant documents, tools that let outputs cause actions, and agents that chain model calls with feedback from the environment until a goal is met.
